                    ...
                    if (data.rating&&data.maxRating&&data.rank&&data.maxRank) {
                        ...
                        if(rank!==prev_rank){
                            ...
                            if(difference>0){
                                ...
                                chrome.notifications.create('ratingUpdate', notificationOptions);
                            }
                            else{
                               if(difference<0){
...
async function fetchNewContestDetailsFromServer() {
    try {
        const response = await fetch('https://cf-reminder-server.onrender.com/newcontestdetails', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
        ...
        if (data.contestInfo.day && data.contestInfo.month && data.contestInfo.year && data.contestInfo.hours && data.contestInfo.minutes && data.contestInfo.ampm && data.contestInfo.contestName) {
            ...
            let currentDateTime = new Date(today.year, today.month - 1, today.day, today.hours, today.minutes, 0, 0);
            let contestDateTime = new Date(contest.year, contest.month - 1, contest.day, contest.hours, contest.minutes, 0, 0);
...
async function scrapeCodeforcesHandle() {
    chrome.tabs.create({ url: 'https://codeforces.com/contests', active: false }, function(tab) {
        chrome.tabs.onUpdated.addListener(function listener(tabId, changeInfo) {
            if (tabId === tab.id && changeInfo.status === 'complete') {
                ...
                chrome.scripting.executeScript({
                    target: { tabId: tab.id },
                    func: () => {
                        const profilehandle = document.querySelector('#header > div.lang-chooser > div:nth-child(2) > a:nth-child(1)');
...
chrome.alarms.create('fetchNewContestDetailsFromServer', { periodInMinutes: 2 });
